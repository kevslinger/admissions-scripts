
FORM_TAB_NAME = "Admissions Form Responses"
DATA_TAB_NAME = "Admissions Decisions"
HYPERLINK = "HYPERLINK"
REDDIT_URL = "https://www.reddit.com/"
REDDITMETIS_URL = "https://redditmetis.com/user/"
GOOGLE_CHROME_PATH = '/app/.apt/usr/bin/google-chrome'
CHROMEDRIVER_PATH = '/app/.chromedriver/bin/chromedriver'
# LOCAL PATHS (for testing)
# GOOGLE_CHROME_PATH = '/usr/bin/google-chrome'
# CHROMEDRIVER_PATH ='./chromedriver'

# Used for things in subreddit_data_scripts, subreddit_purge_scripts, etc.
OUTPUT_DIR = "output_files"
MAIN_TEXT_FILE = "ravenclaw_contributors.txt"
SHADOWBANNED = "ravenclaw_shadowbanned.txt"
UNVERIFIED = "unverified.txt"
UNSUBSCRIBED = "unsubscribed.txt"
RECENCY_CSV = "ravenclaw_recency.csv"
TOWER_RECENCY_CSV = "tower_recency.csv"
